...
class Schedule {
    ...
    private daySchedule: Set<Weekday> = new Set()
    private timeStart: number = 0
    private timeEnd: number = 2359
    ...
    constructor(input: string) {
        ...
        for (let i = 0; i < dateString.length; i++) {
            switch (dateString.charAt(i)) {
                case "M":
                    this.daySchedule.add(Weekday.M)
                    break
                case "T":
                    // Special case for "T" since it can either be Tue or Thu, depending on the next letter
                    switch (dateString.charAt(i + 1)) {
                        case "h":
                            this.daySchedule.add(Weekday.Th)
                            break
                        default:
                            this.daySchedule.add(Weekday.T)
                            break
                    }
                    break
                case "W":
                    this.daySchedule.add(Weekday.W)
                    break
                case "F":
                    this.daySchedule.add(Weekday.F)
                    break
                case "S":
                    this.daySchedule.add(Weekday.S)
                    break
                default:
                    break
            }
        ...
        function convertTo12Hours(input: string): number {
            let output = 0
    ...
    hasConflict(otherSchedule: Schedule): Boolean {
...
class Section {
    ...
    constructor(input: string) {
        ...
        this.isGE = ge_list.some(elem => this.name.toLowerCase().includes(elem.toLowerCase()))
    }
    ...
    hasConflict(other: Section): boolean {
        ...
        for (let otherSchedule of other.schedules) {
            for (let mySchedule of this.schedules) {
                if (mySchedule.hasConflict(otherSchedule)) return true
            }
        }
        return false
    }
